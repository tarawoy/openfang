    ...
    let new_value = if let Some(existing) = tbl.get(last_key) {
        match existing {
            toml::Value::Integer(_) => normalized_value
                ...
                .or_else(|_| normalized_value.parse::<i64>().map(toml::Value::Integer))
        ...
        if let Ok(b) = normalized_value.parse::<bool>() {
            toml::Value::Boolean(b)
        } else if let Ok(i) = normalized_value.parse::<u64>() {
            toml::Value::Integer(i as i64)
        } else if let Ok(i) = normalized_value.parse::<i64>() {
            toml::Value::Integer(i)
        } else if let Ok(f) = normalized_value.parse::<f64>() {
            toml::Value::Float(f)
        } else {
